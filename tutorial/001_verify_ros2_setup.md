...
Ubuntu 버전에 맞춰 지원되며, 이 환경에서는 Ubuntu 20.04에 대응하는 **Foxy Fitzroy**를 사용한다.

| Distro | Ubuntu | 지원 기간 |
|--------|--------|-----------|
| **Foxy** | **20.04** | **2020–2023 (LTS)** ← 이 환경 |
| Humble | 22.04 | 2022–2027 (LTS) |
| Jazzy | 24.04 | 2024–2029 (LTS) |

> **왜 Foxy인가?** Unitree Go2 로봇의 Jetson(Orin Nano)이 Ubuntu 20.04 + ROS 2 Foxy를 사용한다.
> 실기기와 **완전히 동일한 환경**에서 연습해야 나중에 코드를 그대로 배포할 수 있기 때문이다.

## DDS 미들웨어: CycloneDDS

ROS 2 노드 간 통신은 내부적으로 **DDS(Data Distribution Service)** 미들웨어를 통해 이루어진다.
기본 DDS는 FastDDS이지만, 이 환경에서는 **CycloneDDS**를 사용한다.

> **왜 CycloneDDS인가?** Go2 로봇과 `unitree_sdk2`가 CycloneDDS를 통해 통신한다.
> 동일한 DDS를 써야 시뮬레이터와 실기기 모두에서 코드가 그대로 동작한다.
...
이 값이 설정되어 있다는 것은 `source /opt/ros/foxy/setup.bash`가 실행되어
...
`rmw_cyclonedds_cpp`가 출력되면 CycloneDDS가 정상 설정된 것이다.
