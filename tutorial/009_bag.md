...
> ⚠️ `--topics` 옵션은 **Galactic 이상**에서 사용 가능하다.
> Foxy에서는 이 옵션이 없으므로, 특정 토픽만 재생하려면 **녹화 시 토픽을 분리**하는 것이 좋다.

```bash
# Foxy에서의 대안: 녹화할 때 필요한 토픽만 별도로 녹화해둔다
ros2 bag record -o cmd_vel_only /turtle1/cmd_vel
ros2 bag play cmd_vel_only
```

Galactic 이상에서는 아래처럼 재생 시 토픽을 선택할 수 있다:

```bash
# Galactic+ 전용 (Foxy에서는 동작하지 않음)
...
| `ros2 bag play <경로> --topics <토픽>` | 특정 토픽만 재생 (Galactic+, Foxy 미지원) |
